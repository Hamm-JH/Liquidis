...
float GetDepthTexture(float eyeIndex, float2 uv){
    if(eyeIndex == 0.){
        return SAMPLE_DEPTH_TEXTURE(_LeftOrCenterDepthTexture, sampler_LeftOrCenterDepthTexture, uv).r;
    }
    else{
        return SAMPLE_DEPTH_TEXTURE(_RightDepthTexture, sampler_RightDepthTexture, uv).r;
    }
}
